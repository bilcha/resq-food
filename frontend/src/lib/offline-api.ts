...
class OfflineFirstAPI {
  ...
  listings = {
    getAll: async (filters?: ListingFilters): Promise<Listing[]> => {
      ...
      if (!navigator.onLine) {
        console.log(
          'Using local listings data (offline):',
      ...
      try {
        ...
        console.log(
          'Updated with fresh backend data:',
          listings.length,
          'items',
        );
        return listings;
    ...
    getByBusiness: async (businessId: string): Promise<Listing[]> => {
      ...
      if (!navigator.onLine) {
        console.log(
          'Using local business listings for:',
          businessId,
          localListings.length,
          'items',
        );
      ...
      try {
        ...
        console.log(
          'Updated business listings from backend:',
          businessId,
          listings.length,
          'items',
        );
    ...
    create: async (
      data: CreateListingData & { business_id: string },
    ): Promise<Listing> => {
      console.log(
        'OfflineAPI: Creating listing, navigator.onLine =',
        navigator.onLine,
      );
      ...
      if (this.isOfflineOrUnreliable()) {
        ...
        await syncService.queueLocalChange('create', 'listings', {
          ...data,
          _tempId: tempId,
        });
        return tempListing;
      ...
      try {
        ...
        console.warn(
          'OfflineAPI: Backend request failed, falling back to offline:',
          error,
        );
    ...
    update: async (id: string, data: UpdateListingData): Promise<Listing> => {
      console.log(
        'OfflineAPI: Updating listing, navigator.onLine =',
        navigator.onLine,
      );
      ...
      if (this.isOfflineOrUnreliable()) {
        ...
        await syncService.queueLocalChange('update', 'listings', {
          id,
          ...data,
        });
    ...
    delete: async (id: string): Promise<void> => {
      console.log(
        'OfflineAPI: Deleting listing, navigator.onLine =',
        navigator.onLine,
      );
  ...
  business = {
    getAll: async (): Promise<Business[]> => {
      ...
      if (!navigator.onLine) {
        console.log(
          'Using local businesses data (offline):',
      ...
      try {
        ...
        console.log(
          'Updated businesses from backend:',
          businesses.length,
          'items',
        );
        return businesses;
  ...
  async getOfflineStatus(): Promise<{
    ...
    return {
      hasLocalData:
        storageInfo.businessesCount > 0 || storageInfo.listingsCount > 0,
      storageInfo,
    };
  }
    ...
    if (navigator.onLine) {
      try {
        ...
        const result = await originalUploadApi.uploadApi.uploadImage(
          ...
          folder,
          10000,
        ); // 45 second timeout for user-initiated uploads
        return result;
      } catch (error) {
