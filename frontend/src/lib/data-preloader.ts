...
class DataPreloader {
    ...
    try {
      ...
      if (results[2]) {
        if (results[2].status === 'fulfilled') {
          console.log(
            `Preloaded ${results[2].value} business-specific listings`,
          );
        } else {
          console.warn(
            'Failed to preload business listings:',
            results[2].reason,
          );
        }
      }
  ...
  private async preloadBusinessListings(businessId: string): Promise<number> {
    try {
      ...
      console.log(
        `Preloaded ${listings.length} listings for business ${businessId} (all statuses)`,
      );
      return listings.length;
    } catch (error) {
      console.error(
        `Failed to preload listings for business ${businessId}:`,
        error,
      );
      throw error;
    }
  ...
  async updateBusinessContext(context: BusinessContext): Promise<void> {
    ...
    if (
      ...
      if (navigator.onLine) {
        try {
          console.log(
            `Loading listings for newly logged in business: ${context.businessId}`,
          );
          await this.preloadBusinessListings(context.businessId);
          window.dispatchEvent(
            new CustomEvent('business-data-preload-complete'),
          );
        } catch (error) {
