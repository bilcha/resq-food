...
class SyncService {
  ...
  private async syncPendingImages(): Promise<void> {
    ...
    for (const imageData of pendingImages) {
      try {
        console.log(
          `Attempting to upload image ${imageData.id} (attempt ${(imageData.uploadFailedCount || 0) + 1})`,
        );
        ...
        await this.updateListingsWithImageUrl(
          imageData.localUrl,
          result.imageUrl,
        );
        ...
        console.log(
          `Successfully uploaded and cleaned up image ${imageData.id}:`,
          result.imageUrl,
        );
      } catch (error) {
        ...
        console.log(
          `Image ${imageData.id} marked for retry later. Error: ${errorMessage}`,
        );
    ...
    if (failedImages.length > 0) {
      console.log(
        `${failedImages.length} images still pending upload (will retry later)`,
      );
    }
  }

  private async updateListingsWithImageUrl(
    localUrl: string,
    remoteUrl: string,
  ): Promise<void> {
    ...
    const listingsToUpdate = allListings.filter(
      (listing) => listing.image_url === localUrl,
    );
    ...
    for (const listing of listingsToUpdate) {
      ...
      console.log(
        `Updated listing ${listing.id} image URL from local to remote`,
      );
    }
    ...
    console.log(
      `Completed updating ${listingsToUpdate.length} listings with new image URL`,
    );
  }

  private async updatePendingChangesWithImageUrl(
    localUrl: string,
    remoteUrl: string,
  ): Promise<void> {
    const pendingChanges = await offlineDB.getPendingChanges();
    ...
    console.log(
      `Found ${relevantChanges.length} pending changes that need image URL update`,
    );
    ...
    if (table === 'listings') {
      switch (type) {
        case 'create':
          ...
          const createdListing = await (
            await import('./api')
          ).listingsApi.create(createData);
        ...
        case 'update':
          const updatedListing = await (
            await import('./api')
          ).listingsApi.update(data.id, data);
          await offlineDB.saveListing(updatedListing);
          break;
        case 'delete':
      ...
      switch (type) {
        case 'update':
          const updatedBusiness = await (
            await import('./api')
          ).businessApi.updateProfile(data);
  ...
  private async syncFromBackend(): Promise<void> {
    try {
      ...
      const {
        listingsApi: originalListingsApi,
        businessApi: originalBusinessApi,
      } = await import('./api');
  ...
  async getSyncStatus(): Promise<{
    ...
    const allListings = await offlineDB.getListings();
    const tempListings = allListings.filter((listing) =>
      listing.id.startsWith('temp_'),
    ).length;
  ...
  async retryFailedImageUploads(): Promise<void> {
    ...
    try {
      const retryableImages = await offlineDB.getRetryableImages(
        3,
        5 * 60 * 1000,
      );
      ...
      console.log(
        `Background retry: Found ${retryableImages.length} images to retry`,
      );

      for (const imageData of retryableImages) {
        try {
          console.log(
            `Background retry: Uploading image ${imageData.id} (attempt ${(imageData.uploadFailedCount || 0) + 1})`,
          );
          ...
          console.log(
            `Background retry: Successfully uploaded image ${imageData.id}`,
          );
          ...
          console.error(
            `Background retry failed for image ${imageData.id}:`,
            error,
          );
