...
class OfflineDB {
  ...
  async getFailedUploadImages(): Promise<any[]> {
    const db = await this.ensureInit()
    const images = await db.getAll('images')
    return images.filter(img => !img.uploaded && img.uploadFailedCount && img.uploadFailedCount > 0)
  }

  async getRetryableImages(maxRetries: number = 3, minRetryDelay: number = 5 * 60 * 1000): Promise<any[]> {
    const db = await this.ensureInit()
    const images = await db.getAll('images')
    const now = Date.now()
  ...
  async clear(): Promise<void> {
    const db = await this.ensureInit()
    const tx = db.transaction(['businesses', 'listings', 'pending_changes', 'app_state', 'images'], 'readwrite')
  ...
  async getStorageInfo(): Promise<{
    ...
    const db = await this.ensureInit()
    const [businessesCount, listingsCount, pendingChangesCount, imagesCount] = await Promise.all([
      db.count('businesses'),
      db.count('listings'),
